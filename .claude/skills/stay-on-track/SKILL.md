---
name: stay-on-track
description: Keeps a long, multi-step task on-track — breaks out of looping/circular thinking, bounds internal reasoning, watches the context budget, and triggers a clean handoff (via /pause) before the window fills. Trigger on /stay-on-track and proactively when the model is repeating steps, re-reading the same files unchanged, second-guessing in circles, stuck or spinning, or running a long task at risk of exhausting context. Also when the user says it is "looping", "going in circles", "stuck", "repeating itself", or asks for a handoff before running out of context.
---
...
2. **Use this toolkit's `/pause`** to record the pipeline state into `.claude/reports/_state.json` (so `/resume` can pick it up in a fresh session). Don't hand-write the handoff — let `/pause` capture completed steps, what's left, and the context notes.
3. Tell the user plainly: "Context is getting tight — handing off now; save/commit is done and state is written. Start a fresh session and run `/resume` to continue."

> หมายเหตุ: skill นี้ดัดแปลงจาก `qwenchance` ของ [thananon/9arm-skills](https://github.com/thananon/9arm-skills) — เปลี่ยนชื่อให้สื่อความ (ไม่เกี่ยวกับ Qwen) และผูกขั้นตอน hand off เข้ากับ `/pause` `/resume` ของชุดนี้แทน skill `handoff` เดิม
