...
- [แคตตาล็อก Skill (รายละเอียดแต่ละตัว)](#แคตตาล็อก-skill-รายละเอียดแต่ละตัว)
  ...
  - [stay-on-track](#9-stay-on-track)
  - [brainstorming](#10-brainstorming)
...
`brainstorming` (ดัดแปลง — เขียนใหม่ให้เข้ากับ pipeline ของชุดนี้: ถกดีไซน์กับผู้ใช้ก่อนส่งต่อ `system-analyst`)
...
> ส่วนที่พึ่ง CLI ออกและผูก hand off เข้ากับ `/pause` ของชุดนี้แทน — ทั้ง 10 ตัวเป็น guidance ล้วน ไม่พึ่ง CLI
...
### 10. `brainstorming`
> **หมวด:** Collaboration · **ที่มา:** superpowers (ดัดแปลง)

ถกดีไซน์กับผู้ใช้แบบ Socratic เพื่อเปลี่ยนคำขอคลุมเครือให้เป็น**ข้อสรุปดีไซน์ที่ตกลงแล้ว** ก่อนจะเขียน spec หรือโค้ด — แล้วส่งข้อสรุปนั้นต่อให้ `system-analyst` ทำเป็น spec

**จุดสำคัญ (ข้อจำกัดสถาปัตยกรรม):** ต้องรันใน **conversation หลัก (orchestrator)** เท่านั้น เพราะเป็นที่เดียวที่ถามผู้ใช้แล้วได้คำตอบกลับ — **ห้ามใส่ใน subagent** (`system-analyst` รันแยก ได้แค่ list "open questions" แต่ถกกับผู้ใช้กลางทางไม่ได้)

**Workflow:**
1. **Frame** — สรุป "ปัญหา" (ไม่ใช่ solution ที่ผู้ใช้เสนอ) เป็น 1 ประโยค ขอ yes/no
2. **ถามทีละคำถาม** — Socratic ไม่ใช่แบบสอบถาม; ใช้ `AskUserQuestion` เมื่อเป็นตัวเลือกชัดๆ 2–4 ทาง
3. **ตีแผ่ assumption** — พูดสมมติฐานออกมาให้ผู้ใช้ confirm/deny (ใคร ใช้บ่อยแค่ไหน scale เท่าไร fail แล้วยังไง)
4. **เสนอทางเลือก + แนะนำ** — วาง 2–3 ทางพร้อม trade-off แล้ว**เลือกให้ 1 ทาง**พร้อมเหตุผล; ท้าเรื่อง scope (YAGNI) ว่าอะไร**ไม่**ทำ
5. **Converge** — เมื่อ decision สำคัญนิ่งแล้วให้หยุด อย่าวน (ดู [[stay-on-track]])
6. **Design brief** — เขียนสรุปกระชับ (Problem / Decisions / Chosen approach + ทางที่ตัดทิ้ง / Scope in-out / ค้างให้ spec) เป็น input ให้ sa

**เป็นสัดส่วน (ponytail):** คำขอชัด+เล็กพอ → ข้าม หรือยืนยันสั้นๆ ประโยคเดียว; คลุมเครือ/ใหญ่/มีหลายทาง → รัน loop เต็ม

**Trigger:** `/brainstorming`, `/brainstorm` หรือก่อนเขียน spec/เริ่มดีไซน์เมื่อคำขอคลุมเครือ ("ช่วยออกแบบ", "ถกแบบก่อน", "คุยดีไซน์")
**ผูกกับ:** `/spec-only` (Step 1), `/build-feature` (Step 0) — เรียกโดย orchestrator **ไม่ใช่** subagent
...
| `/spec-only` (Step 1) | `brainstorming` | ก่อน dispatch sa เมื่อคำขอคลุมเครือ — **orchestrator เรียกเอง** |
| `/build-feature` (Step 0) | `brainstorming` | ก่อน dispatch sa เมื่อคำขอคลุมเครือ — **orchestrator เรียกเอง** |

> **หมายเหตุ `brainstorming`:** ต่างจากแถวอื่นตรงที่มันถูกเรียกโดย **command orchestrator (ลูปหลัก)** ไม่ใช่ subagent — เพราะต้องคุยโต้ตอบกับผู้ใช้สดๆ ซึ่ง subagent ทำไม่ได้ จึง **ไม่ต้อง**เพิ่ม `Skill` เข้า `tools:` ของ `system-analyst`
...
- Skill ทั้ง 10 ตัวในโฟลเดอร์นี้เป็น **guidance ล้วน ไม่พึ่ง CLI ภายนอก**
